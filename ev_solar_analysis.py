...
results_df = pd.DataFrame({
    'Month': months,
    'Munich Solar (kWh)': monthly_solar_munich.values,
    'Frankfurt Solar (kWh)': monthly_solar_frankfurt.values,
    'Hamburg Solar (kWh)': monthly_solar_hamburg.values,
    'EV Demand (kWh)': monthly_ev,
    'Munich Coverage Simple (%)': [(s / e) * 100 for s, e in zip(monthly_solar_munich.values, monthly_ev)],
    'Munich Coverage Realistic (%)': [(s * self_consumption_rate * total_efficiency / e) * 100
                                       for s, e in zip(monthly_solar_munich.values, monthly_ev)],
})
results_df.to_excel('ev_solar_analysis.xlsx', index=False)
results_df.to_csv('ev_solar_analysis.csv', index=False)
print("\n✅ Excel and CSV exported!")
